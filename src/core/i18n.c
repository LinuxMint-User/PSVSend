...
static const TrEntry s_tr[] = {
    /* ---- 设备页 / 手动扫描横幅 ---- */
    /* 扫描进行中的横幅：只说"探了多少"，不报"已找到 N 台"——设备列表就在正
     * 下方且实时刷新，报在横幅里既冗余、又可能与列表数字不一致。 */
    { "Scanning... %d/%d",
      "扫描中… %d/%d",
      "掃描中… %d/%d",
      "掃描中… %d/%d",
      "スキャン中… %d/%d" },
    /* 这几条画在设备栏标题行右侧，可用宽度 = 栏宽 - 标题宽 - 间距（约 290px），
     * 各语言都要压得住；日语原先的"…台のデバイスを検出"超宽会被整条跳过不画。 */
    ...
    { "Remove", "移除", "移除", "移除", "削除" },
    { "Hold to clear all", "按住清空列表", "按住清空清單", "按住清空清單",
      "長押しで全削除" },
    ...
    { "Parallel uploads", "并发上传数", "並行上傳數", "並行上傳數",
      "並列アップロード数" },
    ...
    /* 设置页值行：列表态确认键=开右侧选项面板，面板态确认键=选定当前档 */
    { "Options", "选项", "選項", "選項", "オプション" },
    { "Select", "选择", "選擇", "選擇", "選択" },
    /* 动作行（主机名/保存目录/主色/检查更新）点了就执行，仍用原来的说法 */
    { "Change", "更改", "變更", "變更", "変更" },
