...
- **页脚提示条（`HintSeg` + `w_page_footer_segs`）**：每段 = 键位图标 + 文案；`icon2` 用于"两个键同一动作"的合并画法「图标 `/` 图标 文案」（两个键都画出来，动作文案只写一次）。排序以**键位默认序**为准——`SELECT、START、方向键1（、方向键2、…）、○、✗、□、△`：先按它排本页**存在**的键，再按**不可变靠左、可变靠右**微调——"可变"只指**动作随焦点 / 运行状态改变**的段（如 `Send↔Remove`、`Cancel↔Done`），这类整段右移；✗ 的 `Up/Back` 只是文案随层级换词、位置与动作恒定，仍按默认序排在 □ 之前。合并段内部的图标同样遵循该序。口径：① 确认键段文案**跟随当前焦点动作**，不写死；② 固定不变的提示靠左、随焦点/状态变化的靠右；③ `HICON_NONE` 必须为 0，无方向位也必须为 0——全仓按 `= { 0 }` 零初始化，反例会让未改动的页面凭空多出图标或灰臂
...
```
src/ui/
├── ui.h            # App 全局状态、页面枚举(PageId)、Input 抽象、页脚提示(HintSeg/HICON_*/HDIR_*)、font_get
├── ui_main.c       # 主循环：输入 → 事件 → 渲染 + 字体按字号分槽（font_get 实现）+ 开屏/预热
├── theme.c/.h      # 主题颜色表 + 切换（Theme 结构，OLED / Yaru）
├── widgets.c       # 文本/列表/按钮/进度条/弹窗/页脚（含键位图标绘制；FONT_PX / FONT_ASC 常量）
├── input.c         # 按键 + 触摸 → 抽象动作
├── pages.c         # 跨页共享状态（进度行清单 xf_*）与开机流程
├── pages_internal.h# 页面层内部接口（两栏布局常量、icon_confirm/icon_back 等共用小工具）
