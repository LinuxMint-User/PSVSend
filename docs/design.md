...
- **已落地（界面，v2.0.0）**：多语言界面（英文 / 简体中文 / 繁中（台灣）/ 繁中（香港）/ 日本語，设置页语言行切换，`i18n.c/h`）+ 设置页底部「关于」区（PSVSend 大字 logo、应用版本与 LocalSend 适配说明）。版本号单源维护：手改 `CMakeLists.txt` 的 `project(VERSION)` 与 `core/config.h` 的 `PSVSEND_APP_VERSION`，SFO `APP_VER` 由 VERSION 自动派生。另有 `PSVSEND_BUILD_TAG`（**构建魔数**，如 `d158`，每次交付构建递增）**只作标识**——boot 日志与设置页「关于」共用这一处来源，「关于」显示 `v2.2.0 (d158)`，装完看一眼即可核对刷入的固件；**不参与版本比较**（更新检查只认 `PSVSEND_APP_VERSION`，见「更新检查」条）。
...
- **已落地（版本标识与更新检查解析，d158）**：`core/config.h` 新增 `PSVSEND_BUILD_TAG`（**构建魔数**，交付构建递增），boot 日志（api.c）与设置页「关于」（pages_settings.c）共用此宏——「关于」显示 `v2.2.0 (d158)`，装完看一眼即可核对固件，不必翻日志；该宏独立于 `PSVSEND_APP_VERSION`、**不参与版本比较**。update.c 的版本宏解析同步收紧：旧实现"找宏名 + 取其后第一个引号串"，只要文件里任何位置出现该 token（注释、其它宏）都可能被带偏（本次加注释时就踩到——定义行之后又出现了一次同名 token）；现改为严格认**定义行**——宏名所在行须以 `#define` 开头（容忍缩进与 `# define`，且 `#define` 与宏名之间是**跳空白**而非固定一个空格，免得将来排版对齐后漏匹配）、宏名须是其后首个记号、其后跳空白即引号，取到的值还须以数字开头；不满足则继续找下一处，全找不到走原有"检查失败"路径。
- **已落地（主页交互完善，d159~d165）**：① **文件栏三角键由"短按直接清空全部"改为长按蓄力触发**——同一个 △ 在设备栏是"扫描"、在文件栏是"清空全部"，短按语义不同，用户容易把清空误认成扫描（本项即由此提出）。现按住 △ **700ms** 蓄满才清空，蓄力期间三角图标周围画进度环（灰底环 + `accent` 进度弧，12 点起顺时针、24 段短直线拼接，项目禁 `vita2d_draw_array`）；松手则进度**渐进回落**（1s，比蓄力稍快），回落中重新按住从当前值续涨；满格触发后上锁至松手（清空后列表为空、该段自身 `dim`）。设备栏**短按扫描不变**。**实现口径：长按判定放输入层**——`input.c` 内维护三角蓄力状态机（与方向键长按重复 `dir_event()` 同属一类，"输入层管长按"本仓已有先例），对外只给 `Input.alt_long`（满格事件）与 `ui_input_alt_charge()`（0~1 进度）；页面**只消费动作、不判断物理键**（曾考虑"页面每帧读物理键自己攒进度"，被否：破坏 ui.h「页面只处理动作」口径，且清空这一副作用会落在 render 里）。满格是**事件**，天然走 `input_page()` 路径，故不需要额外的每帧 tick 钩子；`ui_input_resync()` 同步清零蓄力（解冻不继承按住态）。页脚蓄力环由 `hintbar` 新增 **`w_page_footer_charge(HintKey key, float pct)`** 按需设置（绘制后自动清除，与 `w_clear/w_add` 同模式），**`HintSeg` 结构一字未动**——不给全仓共享结构塞动画量。页脚文案 `Remove all` → `Hold to clear all`（必须明示"要长按"，否则按一下没反应会被当成坏了）。② **页头下细条改为「当前光标项的完整标识行」**：文件栏聚焦 = 选中文件**完整路径**（`w_text_lead`），设备栏聚焦 = **`proto://ip:port`**（`dev_proto`/`dev_ip`/`dev_port`，与官方展示口径接近）——两者都是列表行里看不到的信息（行内只有文件名 / 别名 + 型号）。**扫描进度与结果改画在设备栏标题行右侧**（`Devices (3)` 右边、右对齐、宽度不足时裁尾），据此修掉两个问题：扫描信息与选中路径**共用一行导致叠字**、扫描提示消失时路径**位置跳变**（中途曾试"细条分左右两槽 + 位置固定"，仍不如"信息归位"干净）。空态（无设备）不画状态（栏内 `Scanning network...` 已表达）。扫描中文案简化为「扫描中… N/M」——"找到 N 台"列表里已可见，横幅里重复且可能与列表数字不一致，连带删掉一条词条。③ 扫描完成文案**日语超宽**（标题右侧仅约 290px）会被宽度检查整条跳过不画 → 缩为「スキャン完了、N 台検出」，并把兜底从"挤不下就不画"改成"**裁尾**"，任何语言文案变长都不再静默消失。
...
| 页面 | 内容 | 交互 |
|------|------|------|
| 设备列表（主） | 两栏：在线设备（别名、类型）+ 已选文件清单，左右位置可互换（设置项「布局」）；页头下细条 = **当前光标项的完整标识**（设备栏 = `proto://ip:port`，文件栏 = 完整路径）；**扫描进度 / 结果显示在设备栏标题行右侧** | 上下选条目、左右切栏、方框选文件、确认发送、**设备栏**三角短按扫描、**文件栏**三角**长按 700ms**（图标外画蓄力环）清空全部 |
...
- **页脚提示条独立成模块（`src/ui/hintbar.c/.h`，d106）**：页面只声明"本页用到哪些键、文案、状态"（`HintSeg` 数组），**不关心顺序**；排序、图标选择、排版只在 hintbar 内实现一处，加新按键只需补 `HintKey` 枚举与 `icon_of` / `rank_of` 两张表，各页面零改动。段结构：`key`（+ 可选 `key2` = "两个键同一动作"，画成「键 `/` 键 文案」，两个键都画出、动作文案只写一次）、`text`、`dim`（动作当前不可用 → 灰字灰图标）、`varies`、`dir_off`。**图标由键推出**（`icon_of`：`HKEY_CONFIRM` / `HKEY_BACK` 按当前键位布局在 ○ / ✗ 间取），页面不写图标，消除"键与图标对不上"的非法组合。**可选蓄力环（d159~d165）**：页面在 `w_page_footer_segs()` 之前调 `w_page_footer_charge(key, pct)`，给该键所在段的图标外画一圈进度环（pct 0~1，0 = 不画），绘制后自动清除（与 `w_clear/w_add` 同模式）——`HintSeg` 不增加动画字段，不给全仓共享结构塞一次性渲染参数
- **排序规则（hintbar 内强制，d106/d107）**：默认键序 `SELECT、START、方向键（、…）、○、✗、□、△`——先按它排本页**存在**的键，再按**不可变靠左、可变靠右**微调。"可变"（`HintSeg.varies`）只指**动作随焦点 / 运行状态改变**的段（主页 `Send↔Remove` / `Scan↔Hold to clear all`、进度页 `Cancel↔Done`、文件页 `Select all↔Deselect all`），这类整段右移；✗ 的 `Up/Back` 只是文案随层级换词、动作恒定 → 不算可变，仍按默认序排在 □ 之前。**合并段取两键中靠前的排序位定位，段内两个图标也按同一键序排列**（`seg_rank` / 段内按 rank 交换）——内部图标顺序与整段位置都不随键位布局左右跳。○ / ✗ 两项**按图标符号**定位（○ 恒在 ✗ 前），直接后果：**美式布局下"返回(○)"排在"确认(✗)"之前**，日式布局与旧版相同
