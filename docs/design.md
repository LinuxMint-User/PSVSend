...
- **页脚提示条（`HintSeg` + `w_page_footer_segs`）**：每段 = 键位图标 + 文案；`icon2` 用于"两个键同一动作"的合并画法「图标 `/` 图标 文案」（两个键都画出来，动作文案只写一次）。口径：① 确认键段文案**跟随当前焦点动作**，不写死；② 固定不变的提示靠左、随焦点/状态变化的靠右；③ `HICON_NONE` 必须为 0，无方向位也必须为 0——全仓按 `= { 0 }` 零初始化，反例会让未改动的页面凭空多出图标或灰臂
