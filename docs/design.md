...
- [x] 进度页部分失败的收尾表现：已落地（2026-09-18 d147，见 §1「实现现状」）——总进度条**颜色按终态取**（运行中 / 成功 `accent`、部分失败 `warn`、取消 / 整体失败 `danger`），**填充保持"已传 / 声明总大小"不动**（拉满等于谎报字节数）；高级面板**速度**改用真实已传字节，不再从百分比反推。明确不做"动态分母剔除失败字节"（接收侧无逐文件失败信息，两边收尾会不一致，且失败瞬间分母变小会让条子往前跳）
