...
- **已落地（接收方向）**：对方 prepare-upload → 自动弹接收确认页（Accept / Setup 逐文件勾选 / Reject）→ upload 由 http.c 提供流读回调、receive.c 边收边写 `ux0:data/psvsend/downloads/`（先 `.part` 收完改名，sha256 可选校验，断流/校验失败删残留，开机清扫遗留）。兼容对方无 Content-Length 的 **chunked 流式上传**（http.c 内嵌解码状态机）；同一会话内多文件由对方**并发**发起 upload，接收侧按文件独立流式收体（每文件各自 busy 状态，互不阻塞；d68 修复了 v2.1.0 起后到文件被 409 拒绝的缺陷）。取消/放弃/空闲超时等状态经快照接口给 UI 展示结束原因。announce 已声明 `download:true`。
...
- **已落地（界面交互，d86-d100）**：**扫描中提示**（d99/d100）改为 `Scanning... N of M possible hosts, K found, you could proceed`——分母是 /24 上限、不是"必须探完才能用"的门槛，设备入表即可选可发（knownIps 优先时 ~0.7s 即出现），明说"可继续"以免用户对着 `12/254` 干等；一台未有时不出现该句。发送流程对齐官方顺序——主页改为**两栏**（设备栏 + 已选文件栏），两栏左右位置可在设置项「布局」互换（config `paneSwap`，默认设备在左）；**发送等待独立成页**（`PAGE_SEND_WAIT`）——点设备后停在此页等接收方接受，对方开始上传（`api_send_info` 的 `cur>=0` / `total_sent>0`）才自动切进度页，被拒/连不上则就地红字报错而不误跳进度页（30 分钟 prepare 窗口仅作软兜底，页面本身随时可退）。**页脚提示条口径统一**：确认键段文案跟随当前焦点动作（不写死）、两键同一动作时合并成「图标 / 图标 文案」、固定提示靠左而可变提示靠右；十字键提示改为**方向可用性指示**（`HintSeg.dir_off` 掩码，列表到顶/到底时对应方向变灰）。**设置页**：进入时焦点落在首项（主机名）并滚回顶部；**值行改值方式已于 d145/d146 换成"点行 → 右侧滑出选项面板"**（详见 §5.3 与 §1「实现现状」），取代本节原记录的两条旧口径——"取消选项行左右改值（选项行无左右箭头提示，且上下切换时易误触）"与"「确认键布局」项改用左右方向键切换（避免用确认键修改确认键本身，页脚只替换"改值"那一段）"。
...
- **已落地（发送侧并发上传，d144）**：多文件发送不再逐文件串行——prepare 之后按 config `maxParallel`（设置页「并发上传数」，默认 3、范围 1-6，1 = 退回串行）起 N 个 worker **共享取号**，每个文件一条独立连接（各做一次 TLS 握手），谁先传完谁再取下一个，天然负载均衡。**每文件独立成败**：HTTP 4xx/5xx、对端未回 token、读文件失败只标该文件（`mark_file_failed`），其余照传；**连接级失败（连不上 / 握手失败 / 断流 / 等不到回执）判"对端不在"→ 立即全局中止**——所有阻塞点（connect 轮询 / 握手 / send / recv / 等回执）每轮都查 abort 标志，在途 worker 秒级收手，未派发的文件由监督者统一标失败（实测 `--cut` 断流后 **212ms** 整批收尾，而非各 worker 各等满 6s/30s）。**终态（`finished`/`ok`/`cancelled`）只由监督者 `xfer_thr` 在 join 完全部 worker 之后写一次**，worker 只置文件状态与记错——worker 若自己写 `finished`，UI 会以为任务已结束并允许 `xfer_start` 新任务，而 `g_j` 会被下一次 memset 清空，正在收尾的 worker 就在读一份被清掉的 Job。`total_sent` 改为按增量累加（并发下 `cur` 只有一个，旧的"整表重算"只有一路计入，总进度条不动）。取消通知那条连接改用 `Conn.ignore_flags` 自忽略 cancel/abort，**不再临时清空全局标志**——清标志的那段窗口里其它 worker 会看见"没取消"而继续上传。真机 d144 验证：3 / 6 并发各一轮（6 文件）全成功；`--fail` / `--skip` 单文件失败不牵连其余（终态 1/6 failed）；`--cut` 连接级失败 212ms 整批收尾；6 并发 + 中途取消，对端收到 cancel（HTTP 200）且通知后再无 file done。
- **已落地（设置页值行选项面板，d145/d146）**：设置页全部**值行**（主题 / 外观（深浅）/ 界面语言 / 确认键布局 / 布局 / 并发上传数 / 自动检查）不再走"确认键循环"或"点左右半屏"改值，统一改为**点整行（焦点行按确认同理）→ 右侧滑出选项面板**：面板内 ↑↓ 移光标、确认 = 应用并落盘（**面板不收起**，便于连着试几档）、✗ 或点面板外 = 关且不应用；面板打开期间拖动滚动冻结、列表焦点不动、其余按键吞掉。动作行（主机名 / 保存目录 / 主色 / 检查更新）点了仍直接执行；OLED 主题下「外观」行无档位、开不出面板（页脚确认段画灰）。实现：`settings_change(item, dir)` 拆成 `set_opt_count / set_opt_name / set_opt_cur / set_opt_apply` 四个原语，业务逻辑原样搬入（只把"±1 首尾循环"换成"直接置到第 idx 档"）；面板右贴边 320px、列表行宽 912→576 走 150ms 动画（面板态隐藏滚动条）、滑块式命中区只注册"已滑进来"的部分；页脚两态——列表 `[DPAD] Choose` + `[○] Options` + `[✗] Back`（动作行仍 `Change`），面板 `[○] Select`。连带删除"Tap row halves to change"提示行与 `SET_SLOT_HINT` 槽位，i18n 新增 `Options` / `Select`，`hintbar` 新增 `w_icon_check()`。真机 d145 确认形态（用户提"面板标题与 ✗ 未对齐""应用后立刻收起"两点）→ d146 修掉。
- **已落地（进度页收尾表现，d147）**：主进度条**颜色跟随终态**——运行中 / 全部成功 = `accent`，**部分失败**（`Complete, N failed`）= `warn`，**取消 / 整体失败** = `danger`（此前恒为 accent、与传输中同色，故部分失败停在半路时观感像"还在传"）。**填充不改**：仍按"已传 / 声明总大小"，拉满等于谎报字节数（且与高级面板的 `Files x/y` 自相矛盾）。**"动态分母剔除失败字节"明确不做**——只有发送侧能做（`RecvStatus` 无逐文件失败信息，接收侧只能硬拉），两边收尾会不一致，且文件失败那一瞬间分母变小、条子会往前跳。同时修掉高级面板**速度**：原用 `total * prog_pct / 100` 从百分比**反推**已传字节，1% 粒度截断（传输开头恒读 `0.00 MB/s`）、终态还会虚高，改用真实已传字节（发送 `XferInfo.total_sent` / 接收 `RecvStatus.got_total`），只服务于反推的局部 `total` 一并删掉。接收侧同口径。真机 d147：正常 / `--fail` / `--skip` / `--cut` 四种终态（成功、部分失败 ×2、整体失败）跑通，log 无异常码。
- **已落地（设置页字体预读，d148）**：设置页里切界面语言，原先要现读 16.4MB 拿到对应地区字形（主线程冻结 ~1.6s）。现在**进设置页即起低优先级线程后台预读"另外两份 CJK"**（拉丁与当前语言那份本来常驻），页内切语言只需从 RAM 建字体对象——真机实测相邻两次 `font cjk reload` 间隔 **1~3ms**，十次连续切换（英 / 简 / 繁台 / 繁港 / 日 / 跟随系统）无卡顿；**出设置页停线程并释放非常驻副本**，页外常驻 ≈17MB 与从前一致。页内峰值 ≈ 0.57 + 16.44×3 ≈ **49.9MB**（全项目最大一笔运行期内存），真机三份同驻无 `malloc` 失败。实现要点：字体副本读盘加 `EMPTY/LOADING/READY` 状态机 + **CAS 抢读盘权**（预读线程与"页内切语言"的主线程可能同时要同一份，各读一遍会白占 16MB 且其中一份永远泄漏）；读盘分块（256KB）使线程能在出页时及时收手，且**整份成功才发布**（永不存在半份 buffer）；释放顺序为"停线程 → 等退出 → 才 free"，并按 `cjk_slot_current()` 只清非当前语言那份（天然覆盖"进页后切过语言"：切走的旧语言被释放、新语言保留）。进出用**显式钩子**而非"页边沿"判断——色盘页 / 目录选择页退出时会把 `page` 再设回 `PAGE_SETTINGS`，按页判断会把"进色盘"误判成"离开设置页"→ 掐掉预读、回来重读；另有"接收请求把设置页顶掉"这条路径（`pages_tick` 允许在设置页弹接收确认页）也在 `open_recv_request` 里收尾。`font_reload_cjk` 调用点由 `start_drawing` 之前挪到 **swap + wait_rendering_done 之后**，换字体等待期间屏上已是新语言界面。真机 d148 验证：进页后两份各 ~1.63s 后台读完（同期 `ui: beat` 无缺拍）、页内十次切换无冻结、出页 `slot2/slot3 freed`、再进重新预读，log 全程无异常码。
- **已落地（UI 绘制层参数收敛，d149~d157）**：把散在各页的列表行几何收成**一份常量表**（`ui.h`：页头 / 页脚高、列表可视区 `LIST_TOP`/`LIST_BOTTOM`、`ROW_H`/`ROW_STRIDE`/`ROW_PAD_X`/`ROW_PAD_R`/`ROW_GAP`、行内字号档 `SC_*`）；行型收敛成 `ROW_VALUE`（单行 + 右端）/ `ROW_2LINE`（主上副下）/ `ROW_DENSE`（单行 + 附属进度条）三档，由 `row_geom()` 统一给出锚点 / 字号 / 文本可用宽——`w_row` 原先写死 `r.w-200` 猜右端值宽度，现按 `reserve_right` 实扣；`w_text_right()` 收掉 4~5 种手写右对齐。**长文本三种截断口径定死**：路径 / 目录类 → `w_text_lead`（头部省略、保住末级目录）、文件名类 → `w_text_mid`（中间省略、保住扩展名）、其余 → `w_text_clip`（尾部截断），禁止裸 `w_text` 画长文本。顺带收掉 `area_drag()` 写死 `PANE_TOP`（改由调用方传 `view_top`）与主页两栏**触摸拖动串栏**（`drag_x0/drag_y0` 只在 `drag_start` 帧赋值、而输入结构每 poll 都清零，续帧恒 0 致分栏判定失效），并修掉接收确认页**文件多时预览溢出卡片**与**正常文件名被全报"过长"**（旧写法拿 `strlen` 差推超长，剥掉对端送来的路径前缀后一律变短 → 全报；改用 `recv_sanitize_name` 返回值）。设计口径见 §5.8。
...
| 发送线程（xfer） | `prepare-upload` 一条连接等对方决定；接受后起 N 个 worker 线程并发上传（N = config `maxParallel`，上限 6），join 完全部 worker 后**统一写一次终态**（见 §1「实现现状」） |
...
- 流程：register（可选）→ `prepare-upload` 拿 session（只有这一条连接）→ N 个 worker 并发 `POST upload`（协议 §4.2 明示 `This route can be called in parallel.`）→ cancel 清理
- 按**接收端 announce 的 protocol / port 字段**决定 http/https 与端口——明文即可互通
- 上传：16~64KB 分块读文件写 socket；**每个文件一条连接**，故每文件各做一次 TLS 握手（N 并在途，握手也并行）
- 失败分级：HTTP 4xx/5xx / 无 token / 读文件失败 = 单文件失败（其余继续）；连不上 / 握手失败 / 断流 / 等不到回执 = 连接级失败 → **全局中止**（详见 §1「实现现状」）
...
| 页面 | 内容 | 交互 |
|------|------|------|
| 设备列表（主） | 两栏：在线设备（别名、类型）+ 已选文件清单，左右位置可互换（设置项「布局」） | 上下选条目、左右切栏、方框选文件、确认发送、三角扫描 |
| 文件浏览 | ux0: 目录树，多选文件 | 进入 / 勾选 |
| 发送等待 | 目标设备 + 文件数 / 总大小；等接收方接受，被拒或失败**就地报错**（不跳进度页） | 取消 / 完成后返回 |
| 接收请求确认 | 来者名字/平台 + 文件数 + 预览（含取消态） | 接受 / 设置 / 拒绝；发送方取消 → 单个关闭 |
| 接收设置 | 本次保存目录（可点进目录选择器改本次）+ 逐文件改名（系统键盘 SceIme）/勾选跳过 | 方向键选行、确认勾选、返回继续 |
| 传输进度 | 会话列表（逐文件行 + 细进度条）+ 总进度条（**颜色随终态**：运行中 / 成功 = 主色、部分失败 = 警示色、取消 / 整体失败 = 危险色；填充恒为"已传 / 声明总大小"）+ 高级面板（三角展开：文件计数 / 耗时 / 速度） | 上下滚动列表、三角展开高级面板、取消（结束后变返回） |
| 设置 | 主题、主色（色盘，**仅主题 = 自定义时显示**）、外观（深色/浅色）、布局（两栏左右互换）、**并发上传数**（1-6，默认 3，1 = 退回逐文件串行）、界面语言（跟随系统 / English / 简体中文 / 繁中（台灣）/ 繁中（香港）/ 日本語）、确认键布局、主机名（系统键盘改设备名）、保存目录、检查更新/自动频率 | 上下选行；**值行**点整行（或焦点行按确认）→ 右侧滑出**选项面板**（↑↓ 选档、确认应用且面板不收起、✗ 或点面板外关掉不应用），**动作行**点了直接执行；进入时焦点落在首项并滚回顶部 |
...
- **确认键布局可配置**：美式（X=确认，默认，港版/国行习惯）与日式（O=确认），存 config；触摸不受影响。切换入口是该行自己的**选项面板**（d145/d146）——确认键只负责"开面板 / 应用"、不参与选档，故"用确认键改确认键"的自指负担自然消失（早期为此单开左右方向键，已废）
- **设置页值行 = 右侧选项面板（d145/d146）**：值行点整行（焦点行按确认同理）→ 右侧滑出 320px 面板（列表行宽 912→576、150ms 动画，面板态隐藏滚动条），↑↓ 选档（到顶 / 到底把那一臂画灰，**不循环**）、确认 = 应用并落盘（**面板不收起**，便于连试几档）、✗ 或点面板外 = 关且不应用；面板态拖动滚动冻结、列表焦点不动、其余按键吞掉。2 档同样走面板（交互统一）；动作行不适用（点了直接执行）；OLED 下「外观」行无档位、页脚确认段画灰。页脚文案与实现见 §1「实现现状」
- **页脚提示条独立成模块（`src/ui/hintbar.c/.h`，d106）**：页面只声明"本页用到哪些键、文案、状态"（`HintSeg` 数组），**不关心顺序**；排序、图标选择、排版只在 hintbar 内实现一处，加新按键只需补 `HintKey` 枚举与 `icon_of` / `rank_of` 两张表，各页面零改动。段结构：`key`（+ 可选 `key2` = "两个键同一动作"，画成「键 `/` 键 文案」，两个键都画出、动作文案只写一次）、`text`、`dim`（动作当前不可用 → 灰字灰图标）、`varies`、`dir_off`。**图标由键推出**（`icon_of`：`HKEY_CONFIRM` / `HKEY_BACK` 按当前键位布局在 ○ / ✗ 间取），页面不写图标，消除"键与图标对不上"的非法组合
- **排序规则（hintbar 内强制，d106/d107）**：默认键序 `SELECT、START、方向键（、…）、○、✗、□、△`——先按它排本页**存在**的键，再按**不可变靠左、可变靠右**微调。"可变"（`HintSeg.varies`）只指**动作随焦点 / 运行状态改变**的段（主页 `Send↔Remove` / `Scan↔Remove all`、进度页 `Cancel↔Done`、文件页 `Select all↔Deselect all`），这类整段右移；✗ 的 `Up/Back` 只是文案随层级换词、动作恒定 → 不算可变，仍按默认序排在 □ 之前。**合并段取两键中靠前的排序位定位，段内两个图标也按同一键序排列**（`seg_rank` / 段内按 rank 交换）——内部图标顺序与整段位置都不随键位布局左右跳。○ / ✗ 两项**按图标符号**定位（○ 恒在 ✗ 前），直接后果：**美式布局下"返回(○)"排在"确认(✗)"之前**，日式布局与旧版相同
- **提示条口径**：① 确认键段文案**跟随当前焦点动作**，不写死（接收确认页固定写 "Accept" 会在焦点移到 Reject / Setup 时撒谎）；② 固定不变的提示靠左、随焦点 / 状态变化的靠右；③ `HKEY_NONE` 必须为 0、`dir_off`（`HDIR_*` 位掩码，**0 = 四向全亮**）也以 0 为全亮——全仓按 `= { 0 }` 零初始化，反例会让未改动的页面凭空多出图标或灰臂
- **十字键提示 = 方向可用性指示**：`HintSeg.dir_off` 是"当前按不动的方向"位掩码（0 = 全亮），列表滚到顶/到底时对应方向变灰，用户一眼可知"这个方向按了没用"；灭臂判断**复用各页既有的移动逻辑**（如设置页 `slot_step(sl,-1)==sl` 即"上按不动"），不另写边界判断，保证提示与实际行为一致
...
**字体对象与语言地区（d124-d126）**：字体换 Noto Sans（拉丁 + CJK 简/繁/日三份地区版本，见 §5.5）。字号槽 `g_fs[]` 一次建齐 lat+cjk 两份，CJK 槽按**当前界面语言**算（`cjk_slot_current`）、**不能沿用该次调用的 `cjk` 参数**——建槽通常由"要拉丁"(cjk=0) 的那次调用触发，若沿用参数会把 CJK 也建成拉丁字体，表现为**所有汉字渲染成方块**、且 `font preload` 仍报满数（d125 修复）。换语言时在每帧 `start_drawing` **之前帧外**重建 CJK 字体对象（`font_reload_cjk`：释放旧的、按新槽 `load_slot`）——`vita2d_load_font_mem` 会建 512×512 显存纹理，落在渲染 pass 中途有 GPU crash 风险；首次切到某地区读盘 ~1.6s（16MB），之后常驻 RAM——**该 1.6s 已于 d148 在设置页内消除，见下**。

**设置页字体预读（d148）**：语言行只在设置页，故那 1.6s 只会发生在设置页内。现在**进设置页即起低优先级线程后台预读"另外两份 CJK"**（拉丁与当前语言那份本已常驻），页内切语言退化为从 RAM 建字体对象——真机实测相邻两次 `font_reload_cjk` 间隔 **1~3ms**（十次连续切换无卡顿）；**出页停线程并释放非当前语言那份**，页外常驻 ≈17MB 与从前一致，页内峰值 ≈49.9MB（真机三份同驻无 `malloc` 失败）。读盘侧加 `EMPTY/LOADING/READY` 状态机 + **CAS 抢读盘权**（预读线程与"页内切语言"的主线程可能同时要同一份，各读一遍会白占 16MB 且其中一份永远泄漏）、256KB 分块（使线程能在出页时及时收手）、**整份成功才发布**（永不存在半份 buffer）；释放顺序"停线程 → 等退出 → 才 free"，并按 `cjk_slot_current()` 只清非当前语言那份。进出走**显式钩子**——色盘页 / 目录选择页退出会把 `page` 设回 `PAGE_SETTINGS`，按"页边沿"判断会把进子页误判成离开设置页；"接收请求顶掉设置页"这条路径也在 `open_recv_request` 收尾。`font_reload_cjk` 的调用点同时由 `start_drawing` 前挪到 swap + `wait_rendering_done` 之后（换字体等待期间屏上已是新语言界面）。详见 §1「实现现状」。

### 5.6 文件组织

```
src/ui/
├── ui.h            # App 全局状态、页面枚举(PageId)、Input 抽象、布局常量（§5.8）、行几何 row_geom、font_get
├── ui_main.c       # 主循环：输入 → 事件 → 渲染 + 字体按字号分槽（font_get 实现，字体从内存建；d148 起设置页后台预读另两份 CJK）
├── theme.c/.h      # 主题颜色表 + 切换（Theme 结构，OLED / Yaru / Custom 主色推导）
├── widgets.c       # 文本/列表/按钮/进度条/弹窗（FONT_PX / FONT_ASC 常量、row_geom 实现、长文本三种截断 w_text_lead/mid/clip）
├── hintbar.c/.h    # 底部按键提示条：键位图标绘制 + 段排序 + 排版（HintKey / HintSeg / w_page_footer_segs）
├── input.c         # 按键 + 触摸 → 抽象动作
├── pages.c         # 跨页共享状态（进度行清单 xf_*）与开机流程
├── pages_internal.h# 页面层内部接口（两栏布局常量、key_confirm/key_back 等共用小工具）
├── pages_send.c    # 发送主页（设备栏 + 已选文件栏）/ 文件浏览 / 发送等待
├── pages_recv.c    # 接收确认 / 接收设置 / 系统键盘挂起事务（page_ime_pump）
├── pages_progress.c# 进度页（发送/接收共用）
└── pages_settings.c# 设置页（含色盘页 / 目录选择 / 值行选项面板）
```
...
### 5.8 行几何与长文本口径（d149~d157 定稿）

**只收敛"几何参数入口"，不做样式表覆写层。** 原"样式表"方案（默认一套样式 + 调用点覆写数值 / 字号）能覆盖的只有值行那一族，两行排 / 密集行反正要自绘，多一层"默认表 + 覆写"只会让排查观感问题时多跳一层，故弃用；`RowGeom` 是输出参数，"要偏离就改取回的字段"已经买到了"一处调间距"的收益。

- **几何常量单一来源（`src/ui/ui.h`）**：页头 `HDR_H 52` / 页脚 `FOOTER_H 46`；列表可视区 `LIST_TOP 76` / `LIST_BOTTOM 492` / `LIST_VIEW_H`；行 `ROW_H 56`（可见高）/ `ROW_STRIDE 62`（步进）/ `ROW_PAD_X`、`ROW_PAD_R 24` / `ROW_GAP 12`（主文本与右端元素最小间距）；行内字号档 `SC_TITLE 1.7` / `SC_GROUP 1.05` / `SC_MAIN 1.25` / `SC_SUB 1.0` / `SC_DENSE 1.1`（像素字号 = scale × 20，见 `w_font_px`）。**收敛前同一个概念有 2~5 个取值**：列表顶 76 / 104（主页两栏）/ 68，行步进 62 / 60 / 58，左内边距 20 / 24 / 40 / 48 / 60，右锚点 5 种算法。
- **`row_geom(Rect, RowKind, int reserve_right, RowGeom *g)`（`widgets.c`）**：把行型翻译成 `x_text`（内容左锚点）/ `x_right`（右端元素右边缘）/ `w_text`（主文本可用宽，已扣 `reserve_right`）/ `y_main` / `y_sub` / `main_sc` / `sub_sc` / `row_h`；页面拿结果自己画内容。配套 `w_text_right(x_right, …)` 收掉原先 4~5 种手写右对齐。行型只有 3 档：
  - `ROW_VALUE` —— 单行主文本 + 右端（文字 / 色块 / ✕ / 勾选）：设置页、文件浏览、主页已选文件行、接收设置行；**右端是什么不改变几何**，故行尾控件不是独立行型，走 `reserve_right`。
  - `ROW_2LINE` —— 主上副下：主页设备行、设置页保存目录行。
  - `ROW_DENSE` —— 单行 + 下方附属进度条：进度页。
- **长文本三种截断口径（定死，页面按内容类别选）**：路径 / 目录字符串（保存目录、当前路径、已选文件完整路径）→ `w_text_lead`（**头部省略**，画成 `…/PSPSEND/Download`，末级目录 / 文件名才是要看的那段）；名字类（文件名 / 目录名）→ `w_text_mid`（中间省略，保住扩展名）；其余不受本端控制的文本（对端别名、提示句、平台类型）→ `w_text_clip`（尾部截断）。三者都必须给 `max_w`，**不许用裸 `w_text` 画长文本**（放不下会压到相邻内容上）。
- **不纳入本次收敛**：页头 / 页脚 / 弹窗 / 色盘页各有自己的模块；设置页的分组标题行、logo 行、适配说明行不是"行"，色盘页 HSV 条同理（常量仍在各页内）；主页两栏的 `PANE_*` 仍在 `pages_internal.h`（栏宽 / 栏间距是两栏专有，不参与 `row_geom`）。

## 6. 配置与存储

- `ux0:data/psvsend/config`（JSON，键为驼峰）：alias（设备名）、fingerprint（随机身份串）、port、theme（0=Yaru / 1=OLED / 2=自定义）、lightMode（0=深色 / 1=浅色）、customH / customS / customV（自定义主色 HSV，h 0-359 / s 0-100 / v 0-100，默认 210/65/90；缺字段回默认、越界回默认）、confirmLayout（0=美式 / 1=日式）、paneSwap（主页两栏左右位置：0=设备在左默认 / 1=文件在左）、maxParallel（发送侧并发上传数 1-6，默认 3；越界回默认，1 = 逐文件串行）、lang（0=跟随系统 / 1=English / 2=简体中文 / 3=繁中（台灣）/ 4=繁中（香港）/ 5=日本語；AUTO 由系统语言映射，系统繁体默认台灣）、knownIps（历史在线设备 IP，逗号分隔、LRU、上限 24，主动扫描种子）、updateAuto（更新检查：0=关 / 1=每天 / 2=每周默认 / 3=每月）、updateLast（上次检查更新的网络 unix 秒，自动节流）、saveDir（默认接收目录，默认 `ux0:data/psvsend/downloads`；读回去尾斜杠、非法回退默认）
...
- 大文件流式写盘（内存红线）
- 并发多文件上传（一个 session 多连接）—— **两侧都已落地**：接收侧 d68（按文件独立 busy，逐文件并发收体）、发送侧 d144（N 个 worker 共享取号并发上传，N = config `maxParallel` 上限 6，见 §1「实现现状」）
...
- [x] 非当前语言字体预加载（设置页切语言冻结）：已落地（2026-09-19 d148，见 §1「实现现状」与 §5.5.1）——进设置页即后台预读另两份 CJK，页内切语言由 ~1.6s 读盘退化为从内存建字体对象（真机相邻两次切换间隔 1~3ms）；出页停线程并释放非常驻副本，页外常驻 ≈17MB 不变（页内峰值 ≈49.9MB）
- [x] 进度页部分失败的收尾表现：已落地（2026-09-19 d147，见 §1「实现现状」）——总进度条**颜色按终态取**（运行中 / 成功 `accent`、部分失败 `warn`、取消 / 整体失败 `danger`），**填充保持"已传 / 声明总大小"不动**（拉满等于谎报字节数）；高级面板**速度**改用真实已传字节，不再从百分比反推。明确不做"动态分母剔除失败字节"（接收侧无逐文件失败信息，两边收尾会不一致，且失败瞬间分母变小会让条子往前跳）
- [x] 发送侧多文件并发上传：已落地（2026-09-18 d144，见 §1「实现现状」）——N 个 worker 共享取号、每文件一条独立连接，每文件独立成败、连接级失败立即全局中止，终态由监督者 join 完全部 worker 后统一写一次；并发数做成设置项（默认 3、范围 1-6）
- [x] 设置页值行改值方式：已落地（2026-09-18 d145/d146，见 §1「实现现状」与 §5.3）——值行统一改为"点行 → 右侧滑出选项面板 → ↑↓ 选档 → 确认应用（面板不收起）/ ✗ 关且不应用"，取代"确认键循环"与"点左右半屏"两套旧机制，并连带作废"多档项改左右方向键""值行点左右半屏缺提示"两条诉求
...
- [ ] 接收页"验证"功能（LocalSend 的验证码/校验交互）当前不做：2026-09-09 曾照官方 VerifyPage 实现图案 / 文本比对，真机验证完全无效后**全量回退**——官方客户端**仅对 https 对端**开放验证入口，而接收方向是明文 http（无 TLS server），互操作时收 / 发三个方向的验证按钮全都点不亮，功能无实际用途；待本节「接收方向 TLS 化」落地后再评估
