...
- [x] 界面交互（d86-d98）：主页改为**两栏**（设备栏 + 已选文件栏，左右位置可换，设置项「布局」）；发送等待独立成页（等对方接受，接受后自动进传输页，被拒 / 连不上就地报错）；页脚提示条口径统一（确认键提示跟随当前焦点动作、两键同一动作合并成一段、固定提示靠左而可变提示靠右）；十字键提示改为**方向可用性指示**（列表到顶 / 到底时对应方向变灰，提示按了没用）
